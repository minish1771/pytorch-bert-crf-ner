...
def main(parser):
    ...
    tok_for_vocab = KoBERTTokenizer.from_pretrained('skt/kobert-base-v1')
    _, vocab_of_gluonnlp = get_kobert_model(
        'skt/kobert-base-v1', tok_for_vocab.vocab_file)
    ...
    tr_dl = DataLoader(tr_ds, batch_size=model_config.batch_size,
    ...
    # Model
    model = KobertCRF(config=model_config, num_classes=len(
        tr_ds.ner_to_index), vocab=vocab_of_gluonnlp)
    model.to(device)
    model.train()
            ...
            if (step + 1) % model_config.gradient_accumulation_steps == 0:
                ...
                if model_config.logging_steps > 0 and global_step % model_config.logging_steps == 0:
                    ...
                    if model_config.evaluate_during_training:
                        ...
                        tb_writer.add_scalars('loss', {'train': (
                            tr_loss - logging_loss) / model_config.logging_steps, 'val': eval_summary["eval_loss"]}, global_step)
                        tb_writer.add_scalars(
                            'acc', {'train': tr_acc, 'val': eval_summary["eval_acc"]}, global_step)
                        print("eval acc: {}, loss: {}, global steps: {}".format(
                            eval_summary['eval_acc'], eval_summary['eval_loss'], global_step))
                    print("Average loss: {} at global step: {}".format(
                        (tr_loss - logging_loss) / model_config.logging_steps, global_step))
                ...
                if model_config.save_steps > 0 and global_step % model_config.save_steps == 0:
                    ...
                    output_dir = os.path.join(
                        model_config.output_dir, 'epoch-{}'.format(epoch + 1))
                    ...
                    if is_best:
                        ...
                        checkpoint_manager.save_checkpoint(
                            state, 'best-epoch-{}-step-{}-acc-{:.3f}.bin'.format(epoch + 1, global_step, best_dev_acc))
                        print("Saving model checkpoint as best-epoch-{}-step-{}-acc-{:.3f}.bin".format(
                            epoch + 1, global_step, best_dev_acc))

                        # print classification report and save confusion matrix
                        cr_save_path = model_dir / \
                            'best-epoch-{}-step-{}-acc-{:.3f}-cr.csv'.format(
                                epoch + 1, global_step, best_dev_acc)
                        cm_save_path = model_dir / \
                            'best-epoch-{}-step-{}-acc-{:.3f}-cm.png'.format(
                                epoch + 1, global_step, best_dev_acc)
                        save_cr_and_cm(val_dl, list_of_y_real, list_of_pred_tags,
                                       cr_save_path=cr_save_path, cm_save_path=cm_save_path)
                    else:
                        torch.save(state, os.path.join(output_dir, 'model-epoch-{}-step-{}-acc-{:.3f}.bin'.format(
                            epoch + 1, global_step, eval_summary["eval_acc"])))
                        print("Saving model checkpoint as model-epoch-{}-step-{}-acc-{:.3f}.bin".format(
                            epoch + 1, global_step, eval_summary["eval_acc"]))
...
def evaluate(model, val_dl, prefix="NER"):
    ...
    for batch in tqdm(val_dl, desc="Evaluating"):
        ...
        count_correct += (sequence_of_tags ==
                          y_real).float()[y_real != 2].sum()
